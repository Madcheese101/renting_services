...
class AccountsController(TransactionBase):
	...
	def validate(self):
		...
		if self.doctype in ["Purchase Invoice", "Rent Invoice"]:
			if invalid_advances := [
			...
			pos_check_field = "is_pos" if self.doctype == "Rent Invoice" else "is_paid"
	...
	def validate_deferred_income_expense_account(self):
		field_map = {
			"Rent Invoice": "deferred_revenue_account",
	...
	def validate_all_documents_schedule(self):
		if self.doctype in ("Rent Invoice", "Purchase Invoice") and not self.is_return:
	...
	def validate_party_accounts(self):
		if self.doctype not in ("Rent Invoice", "Purchase Invoice"):
			return

		if self.doctype == "Rent Invoice":
	...
	def validate_inter_company_reference(self):
		...
		if self.is_internal_transfer():
			...
			label = "Delivery Note Item" if self.doctype == "Purchase Receipt" else "Rent Invoice Item"
	...
	def set_missing_item_details(self, for_validate=False):
		...
		if hasattr(self, "items"):
			...
			if self.doctype in ["Quotation", "Sales Order", "Delivery Note", "Rent Invoice"]:
			...
			for item in self.get("items"):
				if item.get("item_code"):
					...
					if self.doctype in ["Purchase Invoice", "Rent Invoice"] and item.meta.get_field(
						"is_fixed_asset"
					):
	...
	def is_pos_profile_changed(self):
		if (
			self.doctype == "Rent Invoice"
			and self.is_pos
			and self.pos_profile != frappe.db.get_value("Rent Invoice", self.name, "pos_profile")
		):
			return True
	...
	def validate_advance_entries(self):
		order_field = "sales_order" if self.doctype == "Rent Invoice" else "purchase_order"
		order_list = list(set(d.get(order_field) for d in self.get("items") if d.get(order_field)))
	...
	def on_cancel(self):
		...
		if self.doctype in ["Rent Invoice", "Purchase Invoice", "Payment Entry", "Journal Entry"]:
	...
	def make_discount_gl_entries(self, gl_entries):
		...
		elif self.doctype == "Rent Invoice":
			enable_discount_accounting = cint(
	...
	def raise_missing_debit_credit_account_error(self, party_type, party):
		...
		db_or_cr = (
			frappe.bold("Debit To") if self.doctype == "Rent Invoice" else frappe.bold("Credit To")
		)
		rec_or_pay = "Receivable" if self.doctype == "Rent Invoice" else "Payable"
	...
	def get_party(self):
		party_type = None
		if self.doctype in ("Opportunity", "Quotation", "Sales Order", "Delivery Note", "Rent Invoice"):
			party_type = "Customer"
	...
	def validate_party_account_currency(self):
		...
		party_account = (
			self.get("debit_to") if self.doctype == "Rent Invoice" else self.get("credit_to")
		)
	...
	def set_payment_schedule(self):
		if self.doctype == "Rent Invoice" and self.is_pos:
			self.payment_terms_template = ""
			return
		...
		if self.doctype in ("Rent Invoice", "Purchase Invoice"):
		...
		if not self.get("payment_schedule"):
			if (
				self.doctype in ["Rent Invoice", "Purchase Invoice"]
	...
	def fetch_payment_terms_from_order(self, po_or_so, po_or_so_doctype):
		"""
		Fetch Payment Terms from Purchase/Sales Order on creating a new Purchase/Rent Invoice.
		"""
	...
	def is_internal_transfer(self):
		...
		"""
		if self.doctype in ("Rent Invoice", "Delivery Note", "Sales Order"):
	...
	def process_common_party_accounting(self):
		is_invoice = self.doctype in ["Rent Invoice", "Purchase Invoice"]
		if not is_invoice:
			return
...
def update_invoice_status():
	...
	for doctype in ("Rent Invoice", "Purchase Invoice"):
...
def update_child_qty_rate(parent_doctype, trans_items, parent_doctype_name, child_docname="items"):
	...
	sales_doctypes = ["Sales Order", "Rent Invoice", "Delivery Note", "Quotation"]
