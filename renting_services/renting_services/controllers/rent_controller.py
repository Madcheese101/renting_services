...
from renting_services.renting_services.controllers.rent_stock_controller import RentStockController

class RentController(RentStockController):
	def __setup__(self):
	...
	def onload(self):
		super(RentController, self).onload()
		if self.doctype in ("Sales Order", "Delivery Note", "Rent Invoice"):
			for item in self.get("items") + (self.get("packed_items") or []):
				item.update(get_bin_details(item.item_code, item.warehouse, include_child_warehouses=True))

	def validate(self):
		super(RentController, self).validate()
		self.validate_items()
	...
	def set_missing_values(self, for_validate=False):
		super(RentController, self).set_missing_values(for_validate)
	...
	def get_already_delivered_qty(self, current_docname, so, so_detail):
		...
		delivered_via_si = frappe.db.sql(
			...
			from `tabRent Invoice Item` si_item, `tabRent Invoice` si
	...
	def update_reserved_qty(self):
		...
		for d in self.get("items"):
			if d.so_detail:
				...
				elif self.doctype == "Rent Invoice" and d.sales_order and self.update_stock:
	...
	def set_incoming_rate(self):
		if self.doctype not in ("Delivery Note", "Rent Invoice"):
			return
	...
	def set_po_nos(self, for_validate=False):
		if self.doctype == "Rent Invoice" and hasattr(self, "items"):
	...
	def set_pos_for_delivery_note(self):
		...
		self.get_po_nos("Rent Invoice", "against_sales_invoice", po_nos)
		self.po_no = ", ".join(list(set(x.strip() for x in ",".join(po_nos).split(","))))
	...
	def validate_for_duplicate_items(self):
		...
		if cint(frappe.db.get_single_value("Selling Settings", "allow_multiple_items")):
			return
		if self.doctype == "Rent Invoice" and self.is_consolidated:
			return
