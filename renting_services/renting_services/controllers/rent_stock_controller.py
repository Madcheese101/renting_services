...
from renting_services.renting_services.controllers.accounts_controller import AccountsController
...
class RentStockController(AccountsController):
	def validate(self):
		super(RentStockController, self).validate()
		...
		for dimension in dimensions:
			...
			if self.doctype in [
				...
				if (
					...
					and self.doctype in ["Rent Invoice", "Delivery Note", "Stock Entry"]
				):
	...
	def validate_inspection(self):
		...
		inspection_fieldname_map = {
			...
			"Rent Invoice": "inspection_required_before_delivery",
		...
		if (
			...
			or (self.doctype in ["Rent Invoice", "Purchase Invoice"] and not self.update_stock)
		):
			return
	...
	def validate_internal_transfer(self):
		if self.doctype in ("Rent Invoice", "Delivery Note", "Purchase Invoice", "Purchase Receipt"):
	...
	def validate_in_transit_warehouses(self):
		if (
			self.doctype == "Rent Invoice" and self.get("update_stock")
	...
	def validate_packed_items(self):
		if self.doctype in ("Rent Invoice", "Delivery Note Item") and self.get("packed_items"):
...
def repost_required_for_queue(doc: RentStockController) -> bool:
