renting_services.PointOfRent.ItemCart = class {
	...
	make_cart_numpad() {
		...
		this.number_pad = new renting_services.PointOfRent.NumberPad({
...
// was POS Invoice
	...
	fetch_customer_details(customer) {
		if (customer) {
			return new Promise((resolve) => {
				frappe.db.get_value('Customer', customer, ["email_id", "mobile_no", "image", "loyalty_program", "special_discount"]).then(({ message }) => {
	...
	// apply_discount(item_row){
	// 	let foundIndex = this.frm.doc.items.findIndex(x => x.name == item_row.name);

	// 	var item = this.frm.doc.items[foundIndex];

	// 	const discount = this.customer_details.special_discount;
	// 	const price_list_rate = item_row.price_list_rate;
	...
	// 	}
	// 	this.frm.doc.items[foundIndex] = item;
	// }
