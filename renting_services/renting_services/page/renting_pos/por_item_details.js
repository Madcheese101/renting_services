renting_services.PointOfRent.ItemDetails = class {
	...
	compare_with_current_item(item) {
		...
		if (item && item.doctype === "Item"){
			item.item_code == this.current_item.item_code;
		}
		else{
	...
	async toggle_item_details_section(item) {
		...
		const doctype = item ? item.doctype : null;
		this.events.toggle_item_selector({minimize:!hide_item_details, doctype: doctype});
		
		this.toggle_component({show:!hide_item_details, doctype: doctype});

		if (item && current_item_changed) {
			this.currency = this.events.get_frm().doc.currency;
			if (item.doctype === "Item"){
				// this.current_rent_item = item;
				this.current_item = item;
				this.render_dom(item);
				this.render_discount_dom(item);
				this.toggle_hide_unhide_elemnts(item.doctype)
				this.render_calendar_section();
			}else{
	...
	render_calendar_section(){
		...
		if (this.occ_date != null){
			...
			let expected_before = frappe.datetime.add_days(this.occ_date, -1);
...
// was POS Invoice
		...
		me.$calendar_container.on('click', '.add-serial-to-cart', async function() {
			...
			// if (me.current_item && me.current_item.doctype != "Item"){
				let args = {
					field: 'qty',
					value: "+1",
					item: me.current_item
				};
				me.events.add_to_cart(args);
			// }
		});
	}
	...
	toggle_hide_unhide_elemnts(doctype){
		if (doctype === "Item"){
			this.$component.find('.form-container').css('display', 'none');
			this.$component.find('.calendar-container').css('display', 'grid');
		}else{
			this.$component.find('.form-container').css('display', 'grid');
			this.$component.find('.calendar-container').css('display', 'none');
		}
	}
}
