...
def check_year_old_unallocated_payments():
    ...
    for payment in result:
        if not payment.cost_center: continue
        if payment.cost_center not in per_cost_center.keys() : per_cost_center[payment.cost_center] = []
        per_cost_center[payment.cost_center].append(payment)
    
    for cost_center_, payments in per_cost_center.items():
        total = 0
        ...
        je.title = _("التسوية الشهرية للعرابين") + " " + cost_center_
